...
impl Problem {
    ...
    pub fn fix_variable(&mut self, var: VariableId, value: f64) -> Result<()> {
...
mod test {
    ...
    fn mipex1() {
        ...
        let solution = problem.solve(ProblemType::MixedInteger).unwrap();
    ...
    fn unfeasible() {
        ...
        let mut problem = problem.set_objective_type(ObjectiveType::Maximize).unwrap();
        assert!(matches!(
